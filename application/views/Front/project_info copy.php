<div class="container-fluid disable-text-selection">
    <div class="col-12 list" data-check-all="checkAll">
        ...
        <div class="row flex-nowrap card-row">
            <?php $gpm = 0; $gpc = 0; 
            foreach ($lists as $list) : $max = 0; $curr = 0; ?>
                        ...
                        <p class="list-item-heading">
                            <?php foreach ($documents as $document) : ?>
                                <?php if ($document->checklists_list_id == $list->id) : ?>
                                    <?php if (!empty($document->file_name)){ $curr++; $gpc++; } ?>
                        ...
                        <footer>
                            <?php if ((($curr / $max) * 100) != 0) : ?>
                                <div class="progress progress1">
                                    <div class="progress-bar bg-<?php if ((($curr / $max) * 100) < 50) {echo "danger";} elseif ((($curr / $max) * 100) < 80) {echo "warning";} else {echo "success";} ?>" style="width: <?= (($curr / $max) * 100) ?>%;"><?= intval((($curr / $max) * 100)) ?>%</div>
                                </div>
                            <?php else : ?>
        ...
        <div class="mt-3">
            <div class="progress-bar grnd-progress-bar row  bg-<?php if ((($gpc / $gpm) * 100) < 50) {echo "danger";} elseif ((($gpc / $gpm) * 100) < 80) {echo "warning";} else {echo "success";} ?>" style="width: <?= (($gpc / $gpm) * 100) ?>%;"><?= intval((($gpc / $gpm) * 100)) ?>%</div>
        </div>
        <?php else: ?>
            <pre>
                <h1 class="mx-auto my-auto mt-5 "><?= $page_text; ?></h1>
            </pre>
        <?php endif; ?>
    </div>
</div>
